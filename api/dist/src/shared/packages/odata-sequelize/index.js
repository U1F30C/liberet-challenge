...
function parseFunction(obj, root, baseOperator, sequelize, tableName) {
    ...
    if (!sequelize.where)
        throw new Error('Sequelize where function not found.');
    const { args } = obj;
    let value = '';
    const operator = obj.func === 'substringof'
        ? getOperator(obj.func, sequelize)
        : baseOperator || getOperator(obj.func, sequelize);
    let key = parseProperty((args.find((t) => t.type == 'property') || {}).name, tableName, true);
    if (!dbFunction.includes(obj.func)) {
        ...
        if (key) {
            ...
            key = parseProperty((func.args.find((t) => t.type == 'property') || {}).name, tableName, true, true);
            addToTree(root, key, sequelize.where(sequelize.fn(getDbFunctionName(func.func), sequelize.col(key)), operator, value));
        }
    }
    else {
        const fn = sequelize.where(sequelize.fn(getDbFunctionName(obj.func), sequelize.col(key)), operator, value);
        addToTree(root, key, fn);
    }
}
function parseProperty(property, tableName, explicitTable = false, isForFunction = false) {
    if (!property) {
        return property;
    }
    if (property.match('/') && !isForFunction) {
        return `$${property.replace('/', '.')}$`;
    }
    if (property.match('/') && isForFunction) {
        return `${property.replace('/', '.')}`;
    }
    if (explicitTable && tableName) {
        if (!isForFunction) {
            return property;
        }
        else {
...
exports.default = (string2Parse, sequelize, tableName) => {
    if (!sequelize)
    ...
    if ('error' in expression)
        throw new Error(`Invalid odata query: ${expression.error}`);
